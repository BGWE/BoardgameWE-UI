                        ...
                        message = { < span id = "error_msg" > Board game successfully added < /span>}
                            ...
                            Grid container spacing = { 16 } > {
                                this.state.hits.map(tile => ( <
                                        ...
                                        component = "h5" > { tile.name } <
                                        /Typography> <
                                        Typography color = "textSecondary" > { tile.year } <
                                        /Typography> <
                                        /CardContent> <
                                        CardActions >
                                        <
                                        Button size = "small"
                                        onClick = {
                                            () => { this.setLoadingStateForTile(tile.id, true);
                                                this.addBoardGame(tile.id); } }
                                        disabled = { tile.isLoading } > {
                                            tile.isLoading ? ( < CircularProgress thickness = { 4 }
                                                size = { 20 }
                                                />) : "Select"} <
                                                /Button> <
                                                /CardActions> <
